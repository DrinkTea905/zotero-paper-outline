...
- **🆕 去除文字空格（小崔定制）**：阅读器工具栏中间（标注工具左边）有个**粉色小猫**图标；中文 PDF 里选中文字、`Ctrl+C` 复制后点它，自动去掉字与字之间的多余空格（中英之间也不留空格、PDF 换行并回成整段，英文单词间正常空格保留），再粘贴即干净。纯规则处理、不调 AI、不联网。开关在「设置 → 高级选项」。
- **🆕 一键清理标注空格（小崔定制）**：阅读器左侧「注释」栏顶部有个醒目的「去除全部标注空格」按钮，点一下把本 PDF **所有标注**（高亮/下划线的文字 + 批注）里的空格一次性去掉，直接改存，无需逐条复制粘贴。
- **🆕 复制 PDF 文件**：把文献的 PDF **文件本身**复制到剪贴板，直接粘贴到文件夹 / 邮件 / 聊天（不是复制文字）。入口：文库里选中条目/附件 `Ctrl+C` 或右键「📋 复制 PDF 文件」、阅读器标签右键、阅读器内右键 / 无选中文字时 `Ctrl+C`。可在「设置 → 高级选项」总开关关闭。
