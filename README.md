...
API URL、API Key 和模型名称会按服务商分别保存在本机；切换服务商时自动恢复各自上次使用的配置。
...
打开 PDF 后，阅读器工具栏会在粉色小猫旁显示两个紧凑的单击按钮：
...
- 重要设置优先展示；
- 文字层级、间距、按钮和勾选框样式统一；
- API URL、API Key 和模型名称按服务商分别保存在本机 Zotero 设置中；
- 高级选项按需展开；
