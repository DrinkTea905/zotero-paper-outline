...
- **多篇 PDF 一次复制**：在文库里多选文献后，可一次复制多份本地 PDF 文件，并自动核对写入数量；缺少本地附件时会明确提示。
- **生成前先测试连接**：设置页和阅读器里都能直接检查 API 是否可用；成功会显示实际模型与响应时间，失败会给出更容易处理的中文提示。
- **设置界面重新整理**：连接、自动处理、生成偏好、实用工具和帮助等内容重新分区，统一文字层级、间距、按钮与勾选框样式，减少文字重叠和寻找成本。

同时改进了模型自动适配、侧栏重新展开后的目录恢复，以及前置印刷目录导致章节全部跳到第一页等问题。

一个**专注、轻量**的 Zotero 插件：用 AI 给论文生成**整篇总结**（存为子笔记）和**层级目录 + 逐节摘要**（显示在阅读器左侧大纲栏，点击跳页）。新文献入库可**自动**生成，也可右键手动。无聊天界面、无多余功能。兼容 **Zotero 7 / 8 / 9**。

## ✨ 功能

- **测试 AI 连接**：可在设置页或阅读器未生成目录的空白栏中一键验证服务商、API Key、模型和接口地址；成功时显示实际模型与响应时间，失败时给出可直接处理的中文提示。
- **🆕 去除文字空格（小崔定制）**：阅读器工具栏中间（标注工具左边）有个**粉色小猫**图标；中文 PDF 里选中文字、`Ctrl+C` 复制后点它，自动去掉字与字之间的多余空格（中英之间也不留空格、PDF 换行并回成整段，英文单词间正常空格保留），再粘贴即干净。纯规则处理、不调 AI、不联网。开关在「设置 → 实用工具」。
- **🆕 一键清理标注空格（小崔定制）**：阅读器左侧「注释」栏顶部有个醒目的「去除全部标注空格」按钮，点一下把本 PDF **所有标注**（高亮/下划线的文字 + 批注）里的空格一次性去掉，直接改存，无需逐条复制粘贴。
- **🆕 复制 PDF 文件**：把文献的 PDF **文件本身**复制到剪贴板，直接粘贴到文件夹 / 邮件 / 聊天（不是复制文字）。入口：文库里单选或多选条目后按 `Ctrl+C`，或右键「📋 复制 PDF 文件」、阅读器**标签**右键。Windows 支持一次复制并粘贴多份文件，写入后会自动校验数量；可在「设置 → 实用工具」关闭。
...
- **阅读器左侧大纲栏**：打开 PDF，左侧出现 AI 生成的层级目录；点条目跳到对应页，翻页时当前章节自动高亮。侧栏收起后重新展开会自动恢复目录；页码定位会排除首页或前部的印刷“目录/目次”页，已有的错误缓存会在打开文献时免费修复。
...
- **模型自动适配**：模型栏留空时自动读取服务商当前可用模型并缓存；正式生成遇到模型调整时会刷新列表并自动重试一次，减少服务商改名或下线模型造成的失效。
...
2. 填 **API Key**（本地 Ollama 免 Key）。
   - DeepSeek：到 <https://platform.deepseek.com> 申请。模型栏建议留空，插件会通过服务商的 `/models` 接口自动选择当前可用模型；只有查询失败时才回退到内置兜底。
   - 其它服务商也会在模型栏留空时自动查询当前模型；如需固定某个模型可手动填写，要用自定义中转可展开「自定义接口地址」。
   - 本地 Ollama：先 `ollama pull qwen3:8b`，URL 默认 `http://localhost:11434/v1/chat/completions`，全离线零费用。
3. 点击 **测试连接**。看到绿色的“连接成功”后再开始生成；测试会发送一次极短请求，可能产生极少量费用。
4. 可选：在「自动处理」中开关自动生成，在「生成偏好」中调整目录层级或提示词；提示词可通过“恢复默认”按钮一键还原。并发数、每块字数和缓存管理位于「高级设置与数据」。

## 🚀 使用

- **入库自动（默认开）**：直接把带 PDF 的文献加进 Zotero，稍候右下角提示「已生成：总结 + 目录」——总结写进子笔记，目录进大纲栏。
- **阅读器内**：打开任意有 PDF 的文献 → 看左侧大纲栏 → 可先点「测试连接」确认接口，再点「📑 生成目录」→ 等生成完，点条目跳页。
...
├─ preferences.js      设置页交互与连接测试状态
├─ preferences.css     设置页视觉样式与深色模式
├─ icons/icon.png      图标
