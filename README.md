...
- **🆕 复制 PDF 文件**：把单篇文献的 PDF **文件本身**复制到剪贴板，直接粘贴到文件夹 / 邮件 / 聊天（不是复制文字）。入口：文库里单选条目或附件后按 `Ctrl+C`，或右键「📋 复制 PDF 文件」、阅读器**标签**右键。受 Zotero 内置剪贴板接口限制，目前不支持一次复制多篇；可在「设置 → 实用工具」关闭。
...
2. 填 **API Key**（本地 Ollama 免 Key）。
   - DeepSeek：到 <https://platform.deepseek.com> 申请；留空模型栏时默认使用 `deepseek-v4-flash`，也可改为 `deepseek-v4-pro`。
   - 其它服务商选中后会自动用其默认 URL / 模型；要换模型可填写「模型（可选）」，要用自定义中转可展开「自定义接口地址」。
